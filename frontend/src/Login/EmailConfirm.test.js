...
const TestWrapper = ({ children, initialEntries = ['/?email=test@example.com'] }) => {
  return (
...
describe('EmailConfirm Component', () => {
  ...
  test('URL 파라미터의 이메일이 표시된다', () => {
    ...
    expect(screen.getByText('user@test.com', { exact: false })).toBeInTheDocument();
    expect(screen.getByText(/로 인증 메일이 전송되었습니다/)).toBeInTheDocument();
  });
  ...
  test('안내 메시지가 올바르게 표시된다', () => {
    ...
    expect(screen.getByText(/이메일을 확인하고 링크를 클릭하여 가입을 완료하세요/)).toBeInTheDocument();
  });
